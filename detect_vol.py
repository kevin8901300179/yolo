...
print(cropped_img.shape)###灰度加切割完成
...
for i in range(len(data1)):
    ...
    if __name__ == '__main__':
        print(len(data1),i)
        # original_depth0 = np.load("/home/kevin/yolov5work/yolov5/data/images/0.npy")##原始
        later_depth0 = original_depth0[ y1 : y2, x1: x2 ] 
        
        # original_depth1 = np.load("/home/kevin/yolov5work/yolov5/data/images/1.npy")##後來
        later_depth1 = original_depth1[ y1 : y2, x1: x2 ] 
        # print('第一次',i)   
        ...
        # print('第一次',i,data1[i],"truth_depth1=",truth_depth1,"truth_depth0=",truth_depth0,"相減=",truth_depth1-truth_depth0)  
       
        # print(float(truth_depth1)-float(truth_depth0))
        if (truth_depth1-truth_depth0) > 0 and truth_depth1 != 0 and truth_depth0 != 0:##深度大於某數在計算 確保誤差
        # if (truth_depth1-truth_depth0) > 70:##深度大於某數在計算 確保誤差
            # print('第一次',len(data1),i,data1[i],"truth_depth1=",truth_depth1,"truth_depth0=",truth_depth0,"相減=",truth_depth1-truth_depth0)  

        # if __name__ == '__main__':
            total.append(truth_depth1-truth_depth0)
            f'{local.append(data1[i])}'
print(len(total))
print('--------------------------------------------------------------')
# print(total)## 距離差
print('--------------------------------------------------------------')
# print(local)## 座標
# depth_image0 = np.load("/home/kevin/yolov5work/yolov5/data/images/0.npy")
for i in range(len(total)):###有深度差大於某數的次數 也為像素數
    ...
    if __name__ == '__main__':
        ...
        # print('第二次總共',len(total),'目前',i,data1[i],local[i],truth_depth0,total[i]) 
        # print(i,local[i],total[i])
        local_depth.append(truth_depth0)

print('--------------------------------------------------------------')
# print(type(local_depth))## 座標原始深度
print('--------------------------------------------------------------')
...
for x in object_name_list:
  if x == 'rice':
      rice_weight = sum(vol)*rice_density
      ...
      print('rice','(',rice_weight,'g )','\n','========= Nutrition Facts =========','\n','總卡路里   碳水化合物  蛋白質  脂肪','\n', Calories,'kcal'  ,carbohydrate ,'g','    ',       protein,'g',' ',  fat,'g')

for x in object_name_list:
  if x == 'bowl':
      egg_weight = sum(vol)*egg_density
      ...
      print('egg','(',egg_weight,'g )','\n','========= Nutrition Facts =========','\n','總卡路里   碳水化合物  蛋白質  脂肪','\n', Calories,'kcal'  ,carbohydrate ,'g','    ',       protein,'g',' ',  fat,'g')
for x in object_name_list:
  if x == 'brocoli':
      brocoli_weight = sum(vol)*brocoli_density
      ...
      print('brocoli','(',brocoli_weight,'g )','\n','========= Nutrition Facts =========','\n','總卡路里   碳水化合物  蛋白質  脂肪','\n', Calories,'kcal'  ,carbohydrate ,'g','    ',       protein,'g',' ',  fat,'g')
for x in object_name_list:
  if x == 'Shrimp':
      Shrimp_weight = sum(vol)*Shrimp_density
      ...
      print('Shrimp','(',Shrimp_weight,'g )','\n','========= Nutrition Facts =========','\n','總卡路里   碳水化合物  蛋白質  脂肪','\n', Calories,'kcal'  ,carbohydrate ,'g','    ',       protein,'g',' ',  fat,'g')
# original_depth = np.load("/home/kevin/yolov5work/yolov5/data/images/0.npy")
# print('------------------------------------')
# print('(',x1,',' ,y1,')','(' ,x2,',', y2,')')   ##第56行
# later_depth = original_depth[ y1 : y2, x1: x2 ] ##
# print(later_depth.shape)
# depth_image = depth_image[82 :380,161:456 ] ##yolo增測為( 161 , 82 )( 456 , 380 )

#######################################################################################
print(f'Done. ({time.time() - t1:.3f}s)')
print('後圖片深度值為零數量',np.sum(original_depth0 == 0))
print('後圖片深度值為零數量',np.sum(original_depth1 == 0))
